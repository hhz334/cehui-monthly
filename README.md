...
| `99_脚本\34_add_toc_links.py <成稿.docx> -o <成稿.docx>` | 目录加书签＋内部超链接（Word/PDF 均可点击跳转），并按页脚回填目录页码 | 就地更新成稿、`_备查\目录跳转链接.md` |
...
- 目录跳转：`34` 只把目录条目的**标题 run** 包进 `w:hyperlink`，制表位与页码留在链接之外；页码按目标页页脚
  “— N —”回填（不是 PDF 物理页序），重排或加页后重跑即可修正错位。
