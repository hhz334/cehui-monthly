...
3. 目录只保留三个板块：政策类、技术应用类、科技前沿类（2026-09-17 起取消“媒体动态类”）；
4. 每条动态按 10 条业务条线打标。
"""
import re
...
# 关键词规则：先命中者优先（政策类 > 科技前沿类 > 技术应用类）
BOARD_RULES = [
    ("政策类", [r"印发", r"发布.{0,10}(?:办法|条例|规划|标准|指南|规定|意见)",
...
def board_of(module, title, summary="", note=""):
    ...
    for board, pats in BOARD_RULES:
        if base == board:
            continue
        scope = title if board == "政策类" else text
        if any(re.search(p, scope) for p in pats):
            return board
    # 政策类兜底校验：标题没有政策特征就不要硬塞进政策类
    if base == "政策类" and not POLICY_TITLE.search(title or ""):
        return "技术应用类"
    return base
